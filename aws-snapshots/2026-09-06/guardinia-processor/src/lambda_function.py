...
CACHE_VERSION = os.environ.get("CACHE_VERSION", APP_VERSION)
...
def detectar_url_promessa(texto: str) -> bool:
    ...
    # A URL e uma chamada para clicar não significam, por si só,
    # promessa ou prêmio. Exige vocabulário de promessa + ação.
    return tem_promessa and tem_acao
...
def buscar_cache(conteudo_hash: str) -> Optional[Dict]:
    ...
    try:
        ...
        item = items[0]
        cache_version = str(item.get("version", ""))
        cache_expires_at = int(item.get("cache_expires_at", 0) or 0)

        if cache_version != CACHE_VERSION:
            logger.info(
                "cache_version_miss | stored=%s | current=%s",
                cache_version,
                CACHE_VERSION
            )
            return None
...
def salvar_cache(
    ...
    try:
        ...
        item = {
            ...
            "ttl": calcular_ttl(),
            "cache_expires_at": int(time.time()) + CACHE_TTL_SECONDS,
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if 'Records' in event:
                            ...
                            for msg in messages:
                                ...
                                if msg.get("type") == "image":
                                    ...
                                    try:
                                        ...
                                        logger.error(traceback.format_exc())
                                        # Propaga a falha para permitir nova tentativa pelo SQS.
                                        raise
            ...
            # Requer ReportBatchItemFailures habilitado no gatilho SQS.
            return {
        ...
        # statusCode HTTP não sinaliza falha de uma invocação SQS.
